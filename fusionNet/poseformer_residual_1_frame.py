...
class ResidualFC(nn.Module):
    def __init__(self,
        ...
        self.dense_in = nn.Sequential(
            ...
            # nn.BatchNorm2d(3),
            nn.LayerNorm([1, 17, self.linear_size]),
...
class Linear(nn.Module):
    def __init__(self, linear_size, p_dropout=0.5):
        ...
        self.layer_norm1 = nn.LayerNorm([1, 17, self.linear_size])

        self.dense_2 = nn.Linear(self.l_size, self.l_size)
        # self.batch_norm2 = nn.BatchNorm2d(243)
        self.layer_norm2 = nn.LayerNorm([1, 17, self.linear_size])
...
class FusionNet(nn.Module):
    def __init__(self, num_frame, num_joints, hidden_chanel, out_chanel):
        ...
        self.regression_head = ResidualFC(
            ...
            in_channel=hidden_chanel,  # for 1 hypothesis
    ...
    def forward(self, x):
        ...
        # # testing with 1 hypothesis
        # feature extractor (pose former)
        B, H, F, J, _ = x.shape
        # out_fea_ext = []
        x = x[:, 0, :, :, :]
        output_extractor = self.feature_extractor(x)
        # out_fea_ext.append(output_extractor)

        # out_fea_ext = torch.cat(out_fea_ext, dim=3)  # → (B, F, J, H*C)

        # # Regression Head
        final_output = self.regression_head(output_extractor)
