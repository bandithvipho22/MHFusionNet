...
if __name__ == '__main__':
    ...
    encoder = Transformer(4, frame*5, frame*2, length=c*num_joint, h=9).cuda()
    norm_1 = nn.LayerNorm(frame*5)

    input = torch.randn(10, 243*5, 17, 2, dtype=torch.float32).cuda()
    # input = input.reshape(10, 243*5, 17, 2)
    input = rearrange(input, 'b f j c -> b (j c) f').contiguous()
