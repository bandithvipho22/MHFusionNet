...
class FusionNet(nn.Module):
    def __init__(self, num_frame, num_joints, hidden_chanel, out_chanel):
        super().__init__()
        # self.feature_extractor = PoseTransformer(num_frame=num_frame, num_joints=num_joints, in_chans=3, out_chans=hidden_chanel, embed_dim_ratio=16,
        #                         depth=2, num_heads=2, mlp_ratio=2., qkv_bias=True, qk_scale=None, drop_path_rate=0.1)
        ...
        #                         depth=2, num_heads=8, mlp_ratio=2.0, qkv_bias=True, qk_scale=None, drop_path_rate=0.2)
        #
        # # Transformer encoder
        # # self.transformer_encoder = TransformerEncoder(depth=2, embed_dim=num_frame*args.num_proposals,
        # #                                               mlp_hidden_dim=num_frame*2, h=9, length=num_joints*hidden_chanel)
        # self.transformer_encoder = TransformerEncoder(depth=2, embed_dim=num_frame * args.num_proposals,
        #                                               mlp_hidden_dim=num_frame * 4, h=9,
        #                                               length=num_joints * hidden_chanel)
        #
        # # Transformer decoder
        # # self.transformer_decoder = TransformerDecoder(depth=2, embed_dim=(num_frame*args.num_proposals) + num_frame,
        # #                                               mlp_hidden_dim=num_frame*2, h=9, length=num_joints*hidden_chanel)
        # self.transformer_decoder = TransformerDecoder(depth=1, embed_dim=(num_frame * args.num_proposals) + num_frame,
        #                                               mlp_hidden_dim=num_frame * 4, h=9,
        #                                               length=num_joints * hidden_chanel)
        ...
        # Transformer decoder
        self.transformer_decoder = TransformerDecoder(depth=4, embed_dim=num_frame * args.num_proposals + num_frame,
                                                      mlp_hidden_dim=num_frame * 2, h=9,
                                                      length=num_joints * hidden_chanel)

        # # Regression Head
        # self.regression_head = nn.Sequential(
        #     # nn.BatchNorm2d(((num_frame*args.num_prout_feature, ....oposals) + num_frame)*args.num_proposals, momentum=0.1),
        #     # nn.LayerNorm(num_joints),
        #     nn.Conv2d(in_channels=((num_frame*args.num_proposals) + num_frame)*args.num_proposals, out_channels=num_frame, kernel_size=1),
        #     # nn.Conv2d(in_channels=num_frame * args.num_proposals, out_channels=num_frame, kernel_size=1),
        #     # nn.BatchNorm2d(num_frame),
    ...
    def forward(self, x):
        ...
        out_fea_reshape = out_fea_ext.reshape(B, H*F, J, C)
        out_fea_reshape = rearrange(out_fea_reshape, 'b f j c -> b (j c) f').contiguous()

        # Transformer Encoder
        out_encoder = self.transformer_encoder(out_fea_reshape)
        global_feature = rearrange(out_encoder, 'b (j c) f -> b f j c', j=J).contiguous()
        # global_feature = out_encoder.reshape(B, H, F, J, C)

        # Transformer decoder
        out_decoders = []
        for i in range(args.num_proposals):
            # f_dec = self.norm_decoder(out_fea_ext[:, i] + self.pose_embed)
            f_dec = out_fea_ext[:, i] + self.pose_embed
            input_decoder = torch.cat((global_feature, f_dec), dim=1)
            input_decoder = rearrange(input_decoder, 'b f j c -> b (j c) f').contiguous()
            out_decoder = self.transformer_decoder(input_decoder)
            out_decoder = rearrange(out_decoder, 'b (j c) f -> b f j c', j=J).contiguous()
            out_decoders.append(out_decoder)  # (b f*h, j, c)
