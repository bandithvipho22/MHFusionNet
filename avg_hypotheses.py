...
def aggregation_train_data():
    ...
    print("======================Hypotheses Aggregation Train set======================")
    log_path = os.path.join('train_hypotheses_aggregation.txt')
    f = open(log_path, mode='a')
    f.write("======================Hypotheses Aggregation Train set======================\n")
    ...
    with torch.no_grad():
        ...
        if not args.no_eval:
            ...
            for cam, batch, batch_2d in test_generator.next_epoch():
                ...
                # predicted_3d_pos[:, :, :, :, 0] = 0
                predicted_3d_pos = predicted_3d_pos[:, 4]

                predicted_3d_pos_mean = predicted_3d_pos.mean(dim=1)
                error = torch.norm(predicted_3d_pos_mean - inputs_3d_single, dim=3)
                mpjpe_mean = error.mean()*1000

                epoch_loss_3d_pos_mean += inputs_3d_single.shape[0] * mpjpe_mean
                N += inputs_3d_single.shape[0]

                # epoch_loss_3d_pos_mean += inputs_3d_single.shape[0] * inputs_3d_single[1] * mpjpe_mean
                # N += inputs_3d_single.shape[0] * inputs_3d_single[1]

                iteration += 1
                # if iteration == 2:
                #     break

            print("======================Hypotheses Average Test set======================")
            # final_avg_mpjpe = epoch_loss_3d_pos_mean.sum().item() / N
            # final_avg_mpjpe = epoch_loss_3d_pos_mean.sum().item() / N
            mean_per_step = epoch_loss_3d_pos_mean / N
            final_avg_mpjpe = mean_per_step.mean().item()
            print("Average MPJPE of Hypotheses: {:.2f} mm".format(final_avg_mpjpe))

            # log_path = os.path.join('test_hypotheses_aggregation_augment.txt')
            # f = open(log_path, mode='a')
            # f.write("======================Hypotheses Aggregation Test set======================\n")
            #
            # print('step %d : Protocol #1 Error (MPJPE) J_Best:' % t, e1[t].item(), 'mm')
            # f.write('step %d : Protocol #1 Error (MPJPE) J_Best: %f mm\n' % (t, e1[t].item()))
            #
...
if __name__ == "__main__":
    ...
    print("\n=== Average hypotheses for testing set ===\n")
