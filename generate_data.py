...
from common.arguments_edited import parse_args  # H:10, K:5
import torch
...
def generate_train_data(model_pos):
    ...
    for cameras_train, batch_3d, batch_2d in train_generator.next_epoch():
        ...
        hypotheses_cpu_train = predicted_3d_pos[4].cpu().numpy()

        hypotheses_cpu_train[:, :, :, 0] = 0

        all_hypotheses_train.append(hypotheses_cpu_train)

        all_inputs_3d_train.append(inputs_3d.cpu().numpy())

        iteration += 1
        # if iteration == 2:
        #     break
    all_hypotheses_train = np.concatenate(all_hypotheses_train, axis=0)
    all_inputs_3d_train = np.concatenate(all_inputs_3d_train, axis=0)

    # Save hypotheses to .npz file
    # output_file = f'frame1_d3dp_5_hypotheses_training.npz'
    output_file = f'H10_d3dp_hypotheses_training.npz'
    np.savez_compressed(output_file, data_hypotheses=all_hypotheses_train)

    output_file_gt = f'H10_d3dp_3d_training_GT.npz'
    np.savez_compressed(output_file_gt, data_gt=all_inputs_3d_train)


def generate_test_data(model_pos):
    with torch.no_grad():
        # load checkpoint d3dp
        ...
        if not args.no_eval:
            ...
            for cam, batch, batch_2d in test_generator.next_epoch():
                ...
                # inputs_3d = torch.stack(inputs_3d, dim=1)

                hypotheses_cpu_test = predicted_3d_pos[:, 4].cpu().numpy()
                # hypotheses_cpu_test = predicted_3d_pos.cpu().numpy()

                hypotheses_cpu_test[:, :, :, 0] = 0
                # hypotheses_cpu_test[:, :, :, :, 0] = 0

                all_hypotheses_test.append(hypotheses_cpu_test)

                all_inputs_3d_test.append(inputs_3d_single.cpu().numpy())

                iteration += 1
                # if iteration == 2:
                #     break
            all_hypotheses_test = np.concatenate(all_hypotheses_test, axis=0)
            all_inputs_3d_test = np.concatenate(all_inputs_3d_test, axis=0)

            # Save hypotheses to .npz file
            output_file = f'H10_d3dp_hypotheses_testing.npz'
            np.savez_compressed(output_file, data_hypotheses=all_hypotheses_test)

            output_file_gt = f'H10_d3dp_3d_testing_GT.npz'
            np.savez_compressed(output_file_gt, data_gt=all_inputs_3d_test)


if __name__ == "__main__":

    # # Generate hypotheses for training set
    # print("\n=== Generating 10 hypotheses for training set ===\n")
    # start_train = time.time()
    # args.test_time_augmentation = False
    # model_pos_train = D3DP(args, joints_left, joints_right, is_train=False, num_proposals=args.num_proposals,
    #                  sampling_timesteps=args.sampling_timesteps)
    # if torch.cuda.is_available():
    #     model_pos_train = nn.DataParallel(model_pos_train)
    # generate_train_data(model_pos=model_pos_train.cuda())
    ...
    print("\n=== Generating 10 hypotheses for testing set ===\n")
    args.test_time_augmentation = True
    start_test = time.time()
    model_pos_test = D3DP(args, joints_left, joints_right, is_train=False, num_proposals=args.num_proposals,
                     sampling_timesteps=args.sampling_timesteps)
    if torch.cuda.is_available():
        model_pos_test = nn.DataParallel(model_pos_test)
    generate_test_data(model_pos=model_pos_test.cuda())
    Testing_time = (time.time() - start_test) / 60
    print("Time spend on generate training:", Testing_time)
