...
# Fusion Network
# from fusionNet.poseformer_residual import FusionNet
# from fusionNet.poseformer_fuse import *
from fusionNet.earlyfuse_residual import FusionNet
hidden_chanel = 24
# # Feature Fusion
# Fusion_Net = FusionNet(num_frame=243, num_joints=17, hidden_chanel=hidden_chanel, out_chanel=3)
# # Early Fusion
Fusion_Net = FusionNet(num_frame=243, num_joints=17, out_chanel=3)
Fusion_Net = nn.DataParallel(Fusion_Net).cuda()
fusion_checkpoint_path = os.path.join(args.checkpoint_fusion, 'v2_best_epoch.bin')  # poseformer + Residual
# fusion_checkpoint_path = os.path.join(args.checkpoint_fusion, 'share_weight_best_epoch.bin')  # poseformer + MLP
fusion_checkpoint = torch.load(fusion_checkpoint_path)
Fusion_Net.load_state_dict(fusion_checkpoint['model_pos'])
Fusion_Net.eval()
    ...
    while epoch < args.epochs:
        ...
        for cameras_train, batch_3d, batch_2d in train_generator.next_epoch():
            ...
            if iteration % 1000 == 0:
                ...
                print_training_time_estimate(epoch, iteration, num_batches, start_time, total_training_start_time, args)
            ...
            # Predict 3D poses
            predicted_3d_pos = model_pos_train(inputs_2d, inputs_3d) #forward pass through model

            loss_3d_pos = mpjpe(predicted_3d_pos, inputs_3d) #calculate mpjpe

            loss_total = loss_3d_pos

            loss_total.backward(loss_total.clone().detach()) #backpropagation
            # loss_total.backward()
...
def evaluate(test_generator, action=None, return_predictions=False, use_trajectory_model=False, newmodel=None):
    ...
    with torch.no_grad():
        ...
        for cam, batch, batch_2d in test_generator.next_epoch():
            ...
            for batch_cnt in range(total_batch):
                ...
                predicted_3d_pos_single = predicted_3d_pos_single[:, 4]  #b, h, f, j, c

                predicted_3d_pos_single[:, :, :, 0] = 0

                predicted_3d_pos_single = Fusion_Net(predicted_3d_pos_single)  #b, f, j, c
                predicted_3d_pos_single = predicted_3d_pos_single[:, None, None].repeat(1, 5, 5, 1, 1, 1)  # from (b, f, j, c) to (b, t, h, f, j, c)
    ...
    log_path = os.path.join(args.checkpoint, 'fusionNet_early_h36m_test_log_H%d_K%d.txt' %(args.num_proposals, args.sampling_timesteps))
    f = open(log_path, mode='a')
    if action is None:
        print('----------')
    else:
